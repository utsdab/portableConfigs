...
class ControlHierarchy(PrimaryRigPart):
    ...
    def _build( self, part, controlShape=control.DEFAULT_SHAPE_DESC, spaceSwitchTranslation=False, parents=(), rigOrphans=False, **kw ):
...
class WeaponControlHierarchy(PrimaryRigPart):
    __version__ = 0
    SKELETON_PRIM_ASSOC = (baseSkeletonPart.SkeletonPart.GetNamedSubclass('WeaponRoot'),)

    def _build( self, part, controlShape=control.DEFAULT_SHAPE_DESC, spaceSwitchTranslation=True, parents=(), **kw ):
        return controlChain( self, part.selfAndOrphans(), controlShape, spaceSwitchTranslation, parents, True, **kw ), ()

def controlChain( rigPart, joints, controlShape=control.DEFAULT_SHAPE_DESC, spaceSwitchTranslation=False, parents=(), rigOrphans=False, **kw ):
    scale = kw[ 'scale' ]
    ...
    for item in joints:
        ctrl = control.buildControl( '%s_ctrl' % item, item, control.PivotModeDesc.BASE, controlShape, size=control.AUTO_SIZE )
        ...
        if not spaceSwitchTranslation:
            control.attrState( ctrl, 't', *control.LOCK_HIDE )
