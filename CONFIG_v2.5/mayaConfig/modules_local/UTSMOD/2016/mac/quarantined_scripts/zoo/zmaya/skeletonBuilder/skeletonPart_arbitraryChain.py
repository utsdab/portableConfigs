...
class ArbitraryChain(SkeletonPart):
    ...
    def _build( cls, parent=None, partName='', jointCount=5, direction='-z', **kw ):
        ...
        directionAxis = rig_utils.Axis.FromName( direction )
    ...
    def _align( self, _initialAlign=False ):
        ...
        if num == 1:
            ...
            midJoint = self[ num / 2 ]
            defaultUpVector = rig_utils.getObjectBasisVectors( self[ 0 ] )[ BONE_OTHER_AXIS ]
            normal = rig_utils.getPlaneNormalForObjects( self.base, midJoint, self.end, defaultUpVector )
