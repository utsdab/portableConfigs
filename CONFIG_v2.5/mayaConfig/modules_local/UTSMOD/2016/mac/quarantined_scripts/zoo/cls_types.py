...
def trackableTypeFactory(metaclassSuper=type):
    ...
    class _TrackableType(metaclassSuper):
        def __new__(cls, name, bases, attrs):
            ...
            # insert the methods above into the newCls unless the names are already
            # taken on the newCls
...
def instanceTrackerTypeFactory(metaclassSuper=type):
    ...
    class _InstanceTrackerType(metaclassSuper):
        def __new__(cls, name, bases, attrDict):
            ...
            # use this as a simple way to generate a unique key - this value gets
            # incremented each time an instance is added to the above weak ref'd dict
            new._NEXT_KEY = 0
        ...
        def IterInstances(self):
            ...
            for key, value in self._INSTANCE_LIST.items():
                ...
                # tidy up the weak value dict as we go - NOTE: this only works if we use
                # items() above not iteritems, as you'd expect
                if value is None:
                ...
                if cls._INTERFACE_CLASS in bases:
                    for methodName in cls._METHODS_TO_IMPLEMENT:
                        ...
                        if methodImplementation.im_func is methodAbstract.im_func:
                            raise TypeError(
                                "The class %s doesn't implement the required method %s!" % (name, methodName))

                        if not compareCallSignatures(methodImplementation, methodAbstract):
                            raise TypeError(
                                "The call signature of %s.%s doesn't match the interface method!" % (name, methodName))
