...
def setDrawOverrideColor(obj, color=17):
    ...
    """
    shapes = []
    if not cmds.objectType(obj, i='nurbsCurve') or not cmds.objectType(obj, i='nurbsSurface') or not cmds.objectType(obj, i='mesh'):
        shapes.append(obj)
    ...
    if shapes:
        for s in shapes:
            conns = cmds.listConnections('%s.drawOverride' % s, s=True)
            if not conns:
                if not color == 0:
                    cmds.setAttr ('%s.overrideEnabled' % s, e=True, l=False)
                    cmds.setAttr ('%s.overrideEnabled' % s, 1)

                    cmds.setAttr ('%s.overrideColor' % s, e=True, l=False)
                    cmds.setAttr ('%s.overrideColor' % s, color)
                else:
                    cmds.color(s)
