...
def MMatrixToPyMatrix(mmatrix, size=4):
    values = []
    for i in xrange(size):
        for j in xrange(size):
            values.append(mmatrix(i, j))

    return vectors.Matrix(values, size)
...
class MirrorNode(MPxNode):
    ...
    # This is the input parent inverse matrix. ie the parent inverse matrix of
    # the transform we want mirrored
    inParentMatrixInv = MObject()

    # Which axis are we mirroring on?
    mirrorAxis = MObject()

    # Boolean to determine whether translation mirroring happens in world space
    # or local space
    mirrorTranslation = MObject()

    # This is the joint orient attribute for the target joint - so it can be
    # compensated for
    ...
    # The rotation order on the target
    targetRotationOrder = MObject()

    # The output translation
    outTranslate = MObject()
    ...
    def compute( self, plug, dataBlock ):
        ...
        R, S = MMatrixToPyMatrix(inWorldMatrix, 3).decompose()  #this gets just the 3x3 rotation and scale matrices
        ...
        matInv = MMatrixToPyMatrix(tgtParentMatrixInv)
...
class ControlPairNode(MPxNode):
    ...
    # This is the axis which things get mirrored across
    axis = MObject()
    flipAxes = MObject()
    ...
    def Init( cls ):
        ...
        cls.mirrorPlane = attrMsg.create( "mirrorPlane", "mrp" )
        cls.addAttribute( cls.controlA )
        cls.addAttribute( cls.controlB )
        cls.addAttribute( cls.mirrorPlane )
