...
# NOTE: the missing 2**1 is deliberate, for whatever reason this isn't used by maya?
KEY_MODIFIERS = SHIFT, CTRL, ALT, PLATFORM_KEY = 2 ** 0, 2 ** 2, 2 ** 3, 2 ** 4
...
def iterAttrs(self, channelBox=True, keyable=True, locked=None, settable=True, userDefined=None):
    '''
    ...
    if keyable is not None:
        cmdKwargs['keyable'] = keyable

    if locked is not None:
        cmdKwargs['locked'] = locked

    if settable is not None:
        cmdKwargs['settable'] = settable

    if userDefined is not None:
        cmdKwargs['userDefined'] = userDefined

    depFn = MFnDependencyNode(self)
    for attrName in cmds.listAttr(self, **cmdKwargs) or []:
        mattr = depFn.attribute(attrName)
        if mattr.isNull():
            continue
        ...
        if channelBox is not None:
            # the logic with channelbox (CB) is weird.  An attribute may not have its CB flag set, but
            # if the attribute is keyable it is ALWAYS visible in the CB.  So only check the CB flag if
            # the attribute isn't keyable
            if not mPlug.isKeyable():
                if mPlug.isChannelBoxFlagSet() != channelBox:
                    continue
...
def setParent(self, newParent, relative=False):
    ...
    dagMod.doIt()
    #cmds.parent( str( self ), str( newParent ), r=relative )
...
def _rename(self, newName):
    ...
    renames the node
    '''
    return cmds.rename(self, newName)
...
### ARRAY CLS CUSTOMIZATIONS ###

__arrayClasses = (MCallbackIdArray, MColorArray, MDagPathArray, MDoubleArray, MFloatArray,
                  MFloatPointArray, MFloatVectorArray, MIntArray, MMatrixArray, MObjectArray,
                  MPlugArray, MPointArray, MTimeArray, MUint64Array, MUintArray, MVectorArray)
# NOTE: these classes are defined in the docs but aren't exposed to python...
# MStringArray

def __arrayClsIter(self):
...
for cls in __arrayClasses:
    cls.__len__ = cls.length
    cls.__iter__ = __arrayClsIter
    cls.__str__ = __arrayStr
    cls.__repr__ = __arrayStr
...
def _aliasName(self):
    #definedAliass = cmds.aliasAttr( self.node(), q=True )
...
def _isHidden(self):
    if self.isElement():
        return bool(cmds.attributeQuery(self.array().longName(), n=self.node(), hidden=True))

    return bool(cmds.attributeQuery(self.longName(), n=self.node(), hidden=True))

MPlug.isHidden = _isHidden

def _isSettable(self):
    return self.isFreeToChange() == MPlug.kFreeToChange

MPlug.isSettable = _isSettable


### OpenMayaAnim CUSTOMIZATIONS ###

def _setCurrentFrame(time):
    '''
    A more convenient way to set scene time than using setCurrentTime directly
    '''
    return MAnimControl.setCurrentTime(MTime(time, MTime.uiUnit()))

MAnimControl.setCurrentFrame = staticmethod(_setCurrentFrame)
...
def ls_(*a, **kw):
    ...
    '''
    return castToMObjects(cmds.ls(*a, **kw) or [])
...
def iterParents(obj):
    ...
    while parent is not None:
        yield parent[0]
        parent = cmds.listRelatives(parent[0], p=True, pa=True)

def iterUp(obj):
    '''
    walks up the hierarchy including the given obj
    '''
    yield obj
    parent = cmds.listRelatives(obj, p=True, pa=True)
    while parent is not None:
...
def getHierarchyRoot(obj):
    '''
    returns the top most node of the hierarchy the given object is in
    '''
    for obj in iterUp(obj):
        continue
...
def iterNonUniqueNames():
    # NOTE: only dag objects can have non-unique names...  despite the fact that the
    # hasUniqueName method lives on MFnDependencyNode (wtf?!)
    for mobject in iterDags():

        # thankfully instantiating MFn objects isn't slow - just MObject and MDagPath
        if not MFnDependencyNode(mobject).hasUniqueName():
            yield mobject

def fixNonUniqueNames():
    rename = cmds.rename
    for mobject in iterNonUniqueNames():
        name = MFnDependencyNode(mobject).name()
