...
mayaVer = int(mel.eval('getApplicationVersionAsFloat'))

class MelUIError(Exception): pass

#this maps ui type strings to actual command objects - they're not always called the same
TYPE_NAMES_TO_CMDS = { u'staticText': cmds.text,
                       u'field': cmds.textField,
                       u'cmdScrollField': cmds.scrollField,
                       u'commandMenuItem': cmds.menuItem,
                       u'dividorMenuItem': cmds.menuItem }
...
WIDGETS_WITHOUT_DOC_TAG_SUPPORT = [ cmds.popupMenu ]
...
class BaseMelUI(unicode):
    ...
    class AButtonClass(BaseMelUI):
    	WIDGET_CMD = cmds.button

    aButton = AButtonClass(parentName, label='hello')
    aButton(edit=True, label='new label!')
    '''

    __metaclass__ = cls_types.trackableTypeFactory(cls_types.instanceTrackerTypeFactory())

    #this should be set to the mel widget command used by this widget wrapped - ie cmds.button, or cmds.formLayout
    WIDGET_CMD = cmds.control
    ...
    def Exists(cls, theControl):
        ...
        return cmds.control(theControl, q=True, exists=True)
    @classmethod
    ...
    def _executeCB(self, cbFlagName=None):
        ...
        if callable(cb):
            try:
                ...
                logger.error("The %s callback failed" % cbFlagName, exc_info=1)
    ...
    def processEvent(self, methodName, methodArgs, methodKwargs):
        ...
        if callable(method):
            try:
                ...
                logger.error('Event Failed: %s, %s, %s' % (methodName, methodArgs, methodKwargs), exc_info=1)
        else:
            self.parent.processEvent(methodName, methodArgs, methodKwargs)
    ...
    def getParent(self):
        ...
        '''
        return self.parent  #cmds.control(self, q=True, parent=True)
        ...
        try:
            ...
            while True:
                ...
                #in which case, try to cast the widget as a BaseMelUI instance and yield that instead
                    ...
                    parent = BaseMelUI.FromStr(cmds.control(parent, q=True, parent=True))
        ...
        for parent in self.iterParents():
            if exactMatch:
                ...
                if issubclass(parent.__class__, parentClass):
                    return parent
    def exists(self):
        return cmds.control(self, ex=True)
    def delete(self):
        cmds.deleteUI(self)
    def setSelectionChangeCB(self, cb, compressUndo=True, **kw):
        ...
        '''
        return cmds.scriptJob(compressUndo=compressUndo, parent=self, event=('SelectionChanged', cb), **kw)
    def setSceneChangeCB(self, cb, compressUndo=True, **kw):
        ...
        '''
        return cmds.scriptJob(compressUndo=compressUndo, parent=self, event=('SceneOpened', cb), **kw)
    def setTimeChangeCB(self, cb, compressUndo=True, **kw):
        ...
        '''
        return cmds.scriptJob(compressUndo=compressUndo, parent=self, event=('timeChanged', cb), **kw)
    def setAttributeChangeCB(self, attrpath, cb, compressUndo=True, allChildren=False, disregardIndex=False, **kw):
        ...
        '''
        return cmds.scriptJob(compressUndo=compressUndo, parent=self, attributeChange=(attrpath, cb), allChildren=allChildren, disregardIndex=disregardIndex, **kw)
    def setDeletionCB(self, cb, compressUndo=True, **kw):
        '''
        define a callback that gets triggered when this piece of UI gets deleted
        '''
        return cmds.scriptJob(compressUndo=compressUndo, uiDeleted=(self, cb), **kw)
    def setUndoCB(self, cb, compressUndo=True, **kw):
        '''
        define a callback that gets triggered when an undo event is issued
        '''
        return cmds.scriptJob(compressUndo=compressUndo, parent=self, event=('Undo', cb), **kw)
    @classmethod
    def FromStr(cls, theStr):
        ...
        try:
            uiTypeStr = cmds.objectTypeUI(theStr)
        except RuntimeError:
            try:
                uiTypeStr = cmds.objectTypeUI(theStr.split('|')[ -1 ])
            except RuntimeError:
                uiTypeStr = ''

        uiCmd = TYPE_NAMES_TO_CMDS.get(uiTypeStr, getattr(cmds, uiTypeStr, cmds.control))
        ...
        try:
            new.parent = cmds.control(theStr, q=True, parent=True)
...
class MelRowLayout(BaseMelLayout): WIDGET_CMD = cmds.rowLayout
MelRow = MelRowLayout

class MelGridLayout(BaseMelLayout):
    WIDGET_CMD = cmds.gridLayout
...
class MelPaneLayout(BaseMelLayout):
    ...
    def __init__(self, parent, configuration=None, *a, **kw):
        ...
        if self.PREF_OPTION_VAR:
            if cmds.optionVar(ex=self.PREF_OPTION_VAR):
                storedSize = cmds.optionVar(q=self.PREF_OPTION_VAR)
                for idx, size in enumerate(misc.iterBy(storedSize, 2)):
    ...
    def on_resize(self, *a):
        if self.PREF_OPTION_VAR:
            ...
            cmds.optionVar(clearArray=self.PREF_OPTION_VAR)
            for i in size:
                cmds.optionVar(iva=(self.PREF_OPTION_VAR, i))

class MelFrameLayout(BaseMelLayout):
    WIDGET_CMD = cmds.frameLayout
...
class BaseMelWidget(BaseMelUI):
    def setValue(self, value, executeChangeCB=True):
        try:
            ...
            logger.error('Running setValue method using %s command' % self.WIDGET_CMD, exc_info=1)
            raise
...
class MelRadioCollection(unicode):
    WIDGET_CMD = cmds.radioCollection
...
class MelIconRadioButton(BaseMelWidget, _Image):
    WIDGET_CMD = cmds.iconTextRadioButton

class MelIconRadioCollection(MelRadioCollection):
    WIDGET_CMD = cmds.iconTextRadioCollection
    BUTTON_CLS = MelIconRadioButton

class MelSeparator(BaseMelWidget):
    WIDGET_CMD = cmds.separator

class MelIntField(BaseMelWidget):
    WIDGET_CMD = cmds.intField
    DEFAULT_WIDTH = 30

class MelFloatField(BaseMelWidget): WIDGET_CMD = cmds.floatField
class MelTextField(BaseMelWidget):
    WIDGET_CMD = cmds.textField
    DEFAULT_WIDTH = 150
    KWARG_VALUE_NAME = 'tx'
...
class MelObjectSelector(MelFormLayout):
    ...
    def buildMenu(self, menu, menuParent):
        ...
        obj = self.getValue()
        enabled = cmds.objExists(obj) if obj else False
...
class _BaseSlider(BaseMelWidget):
    ...
    def __init__(self, parent, minValue=0, maxValue=100, defaultValue=None, *a, **kw):
        ...
        self._initialUndoState = cmds.undoInfo(q=True, state=True)
...
class MelFloatSlider(_BaseSlider):
    WIDGET_CMD = cmds.floatSlider

class MelIntSlider(_BaseSlider):
    WIDGET_CMD = cmds.intSlider
...
class MelTextScrollList(BaseMelWidget):
    ...
    WIDGET_CMD = cmds.textScrollList
...
class MelObjectScrollList(MelTextScrollList):
    ...
    def itemAsStr(self, item):
        ...
        if self.DISPLAY_NICE_NAMES:
            itemStr = str_utils.camelCaseToNice(itemStr)
...
class MelSetMemebershipList(MelObjectScrollList):
    ...
    def setSets(self, objectSets):
        ...
        self.update()
    def getAllItems(self):
        items = cmds.sets(self.objectSets, q=True) or []
        items.sort()

        return misc.removeDupes(items)
    ...
    def _removeItems(self, items):
        if items:
            for objectSet in self.objectSets:
                cmds.sets(items, remove=objectSet)
    ...
    def filterItems(self, items):
        ...
        for item in items:
            if nodeType in self.ALLOWED_NODE_TYPES:
                if cmds.objectType(item, isAType=nodeType):
    ...
    def on_addItem(self, *a):
        self._addItems(cmds.ls(sl=True))
    def on_replaceItem(self, *a):
        self._setItems(cmds.ls(sl=True))
    def on_removeItem(self, *a):
        self._removeItems(cmds.ls(sl=True))
...
class _MelBaseMenu(BaseMelWidget):
    ...
    def clear(self):
        for menuItem in self.getMenuItems():
            cmds.deleteUI(menuItem)

class MelMenu(_MelBaseMenu):
    WIDGET_CMD = cmds.menu
    DYNAMIC = True
...
class MelRadioMenuCollection(unicode):
    WIDGET_CMD = cmds.radioMenuItemCollection
...
class MelIteratorUI(object):
    def __init__(self, iterableObject, maxRange=None, **kw):
        ...
        cmds.progressWindow(progress=0, isInterruptable=True, minValue=0, maxValue=maxRange, **kw)
    def __iter__(self):
...
class BaseMelWindow(BaseMelUI):
    ...
    args to the cmds.window maya command:
    ...
    aWindow = cmds.window()
    cmds.window(aWindow, q=True, exists=True)
    '''
    ...
    def Exists(cls):
        ...
        '''
        return cmds.window(cls.WINDOW_NAME, ex=True)
    @classmethod
    def Get(cls):
        '''
    ...
    def __new__(cls, *a, **kw):
        ...
        if cmds.window(cls.WINDOW_NAME, ex=True):
            cmds.deleteUI(cls.WINDOW_NAME)

        new = unicode.__new__(cls, cmds.window(cls.WINDOW_NAME, **kw))
        cmds.window(new, e=True, docTag=cls.__name__)   #store the classname in the
        ...
        if cls.HELP_MENU:
            ...
            MelMenuItem(helpMenu, l="Help...", en=helpPage is not None, c=lambda x: cmds.showHelp(helpPage, absolute=True))
    ...
    def __call__(self, *a, **kw):
        return cmds.window(self, *a, **kw)
    def setTitle(self, newTitle):
        cmds.window(self.WINDOW_NAME, e=True, title=newTitle)
    def getMenus(self):
    ...
    def getLayout(self):
        ...
        existingLayouts = cmds.lsUI(controlLayouts=True, long=True)
    ...
    def exists(self):
        return cmds.window(self.WINDOW_NAME, q=True, ex=True)
