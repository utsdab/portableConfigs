...
def isMesh(item):
    shapes = cmds.listRelatives(item, shapes=True, pa=True)
    if shapes is None:
        return None
...
class LockJointsLayout(MelFormLayout):
    ...
    def __init__(self, parent):
        ...
        self.POP_tsl = cmds.popupMenu(b=3, p=self.UI_tsl, pmc=self.build_tslMenu)

        self.UI_lock = cmds.button(l="lock", c=self.on_lock)
        self.UI_unlock = cmds.button(l="unlock", c=self.on_unlock)

        self.UI_lockAll = cmds.button(l="LOCK ALL", c=self.on_lockAll)
        self.UI_unlockAll = cmds.button(l="UNLOCK ALL", c=self.on_unlockAll)
    ...
    def attachToMesh(self, mesh):
        ...
        joints = cmds.skinCluster(self._skinCluster, q=True, inf=True)

        self._prefix = str_utils.getCommonPrefix(joints)
    ...
    def smoothBetweenSelectedJoints(self):
        ...
        # grab initial state
        initialLockState = [cmds.getAttr('%s.liw' % j) for j in self.UI_tsl]
        initMode = cmds.artAttrSkinPaintCtx(cmds.currentCtx(), q=True, sao=True)
        initOpacity = cmds.artAttrSkinPaintCtx(cmds.currentCtx(), q=True, opacity=True)
        initValue = cmds.artAttrSkinPaintCtx(cmds.currentCtx(), q=True, value=True)
        ...
        #perform the smooth
        cmds.artAttrSkinPaintCtx(cmds.currentCtx(), e=True, sao='smooth')
        j = self.iterSelected().next()
        if j is not None:
            cmds.artAttrSkinPaintCtx(cmds.currentCtx(), e=True, value=1, opacity=1, clear=True)


        #restore state
        cmds.artAttrSkinPaintCtx(cmds.currentCtx(), e=True, sao=initMode)
        cmds.artAttrSkinPaintCtx(cmds.currentCtx(), e=True, opacity=initOpacity)
        cmds.artAttrSkinPaintCtx(cmds.currentCtx(), e=True, value=initValue)
    ...
    def syncSelection(self):
		...
		'''
        cur = cmds.artAttrSkinPaintCtx(cmds.currentCtx(), q=True, inf=True)
        if cur is None:
            return
    ...
    def build_tslMenu(self, *a):
        cmds.setParent(a[0], m=True)
        cmds.menu(a[0], e=True, dai=True)

        numSelected = sum(( 1 for j in self.iterSelected() ))

        cmds.menuItem(l="Select Verts For Joint", c=self.on_selectVerts)
        if numSelected > 1:
            cmds.menuItem(l="Select Verts Shared By Selected", c=self.on_selectIntersectingVerts)

        cmds.menuItem(l="Select Whole Mesh", c=self.on_selectMesh)
        cmds.menuItem(d=True)

    #cmds.menuItem( l="Smooth Between Selected Joints", c=self.on_smooth )
    ...
    def on_selectVerts(self, *a):
        ...
        verts = []
        for j in selJoints:
            verts += mesh_utils.jointVertsForMaya(j)

        if verts:
            cmds.hilite([self._mesh])
            cmds.select(verts)
    ...
    def on_selectIntersectingVerts(self, *a):
        ...
        for j in selJoints:
            jointVerts[j] = verts = mesh_utils.jointVertsForMaya(j)
            allVerts += verts
...
class SkinButtonsLayout(MelHLayout):
    def __init__(self, parent):
        ...
        self.UI_skinOff = MelButton(self, l='Turn Skinning Off', c=self.on_skinOff)
        self.UI_skinOn = MelButton(self, l='Turn Skinning On', c=self.on_skinOn)
        MelButton(self, l='Reset All Skin Clusters', c=self.on_resetSkin)
...
class WeightsToOtherLayout(MelHLayout):
    def __init__(self, parent):
        ...
        self.UI_toParent = MelButton(self, l='Weighting To Parent Joint',
                                     ann='Transfers weighting from all selected joints to their respective parents',
                                     c=self.on_toParent)
        self.UI_toOther = MelButton(self, l='Weighting To Last Selected Joint',
                                    ann='Transfers the weighting from the selected joints to the joint that was selected last',
                                    c=self.on_toOther)
        self.layout()
    ...
    def on_toParent(self, e=None):
        ...
        for s in sel:
            mesh_utils.weightsToParent(s)

    def on_toOther(self, e=None):
        sel = cmds.ls(sl=True, type='transform') or []
        lastSelectedJoint = sel.pop()
        for s in sel:
            mesh_utils.weightsToOther(s, lastSelectedJoint)
...
class SkinWeightsLayout(MelColumnLayout):
    ...
    def getFilepath(self):
        return path.Path(self.UI_file.getValue())
    ...
    def build_popup(self, parent, *a):
        ...
        thisFile = path.Path(cmds.file(q=True, sn=True))
        ...
        dir = thisFile.up()
        curFile = self.getFilepath()

        for f in dir.files():
            if f.hasExtension(skinWeights.EXTENSION):
                cmds.menuItem(l=f.name(), cb=f == curFile, c=misc.Callback(self.UI_file.setValue, f))

        cmds.menuItem(d=True)
        cmds.menuItem(l="browse", c=self.on_browseWeightFile)
        cmds.menuItem(d=True)
        cmds.menuItem(l="clear", c=lambda *a: self.UI_file.setValue(''))
    ...
    def on_meshMap(self, *a):
        try:
            ...
            data = skinWeights.WeightSaveData(filepath.unpickle())
            meshes = list(data.getUsedMeshes())

            sceneMeshes = cmds.ls(typ='mesh')
            if sceneMeshes:
                sceneMeshes = cmds.listRelatives(sceneMeshes, p=True, pa=True)

            mapping = str_utils.Mapping(meshes, sceneMeshes)
    ...
    def on_jointMap(self, *a):
        try:
            ...
            sceneJoints = cmds.ls(typ='joint')
            mapping = str_utils.Mapping(joints, sceneJoints)
    ...
    def on_browseWeightFile(self, *a):
        startDir = skinWeights.getDefaultPath().up()

        filepath = cmds.fileDialog(directoryMask=startDir / "/*.weights")
        if filepath:
            cmds.textField(self.UI_file, e=True, tx=filepath)

    def on_storeA(self, *a):
        kw = {}
        if self.UI_file.getValue():
            kw['filepath'] = self.getFilepath()

        skinWeights.saveWeights(cmds.ls(sl=True), **kw)
    ...
    def on_storeB(self, *a):
        ...
        joints = cmds.ls(type='joint', r=True)
        jointMeshes = misc.removeDupes(cmds.listRelatives(joints, ad=True, pa=True, type='mesh'))
    ...
    def on_restore(self, *a):
        filepath = None
        if self.UI_file.getValue():
            filepath = path.Path(self.UI_file.getValue())

        skinWeights.loadWeights(cmds.ls(sl=True),
                                filepath,
                                True,
