...
class SplineIK(PrimaryRigPart):
    __version__ = 2
    SKELETON_PRIM_ASSOC = (baseSkeletonPart.SkeletonPart.GetNamedSubclass('ArbitraryChain'),)
    PRIORITY = 11
    ...
    def _build(self, skeletonPart, allowFixedLength=True, constrainBetween=True, **kw):
        ...
        parent(miscGrp, worldPart.getNamedNode('parts'))
...
def buildControls(objs, controlParent=None, name='control', midName='midControl', allowFixedLength=True, constrainBetween=True, constrainBetweenMid=True, **kw):
    ...
    control.attrState((fittedCurve, linearCurve), ('t', 'r', 's'), *control.LOCK_HIDE)
    ...
    for n, obj in enumerate(objs):
        ...
        ctrlColour = control.ColourDesc('darkblue') if isHalfCtrl else control.ColourDesc('blue')
        nControl = control.buildControl(ctrlName, obj, control.PivotModeDesc.MID, 'sphere', ctrlColour, constrain=False, parent=controlParent, scale=ctrlScale, asJoint=True)  #need to be joints to use for skinning below
    ...
    if constrainBetween:
        ...
        if numObjs >= 5 and constrainBetweenMid:
            lengths = [ rig_utils.betweenVector(controls[0], c).get_magnitude() for c in controls[ 1:halfIdx+1 ] ]
            ...
            lengths = [ rig_utils.betweenVector(controls[0], c).get_magnitude() for c in controls[ halfIdx:-1 ] ]
            lengthSum = sum(lengths)
            ...
            lengths = [ rig_utils.betweenVector(controls[0], c).get_magnitude() for c in controls[ 1: ] ]
            lengthSum = sum(lengths)
    ...
    for n in range(numObjs - 1):
        ...
        v = rig_utils.betweenVector(obj, nextObj)

        #now get all the axis information to build an aim constraint
        aimAxis = rig_utils.getObjectAxisInDirection(obj, v)

        otherAxes = aimAxis.otherAxes()
        upAxis = otherAxes[0]
        upVector = rig_utils.getObjectBasisVectors(obj)[ upAxis ]
        upObj = controls[n]
        upObjAxis = rig_utils.getObjectAxisInDirection(upObj, upVector)
    ...
    if allowFixedLength:
        for j in proxies:
            fixedJ = baseSkeletonPart.createJoint('%s_fixedLength' % j)
        ...
        splineIkHandle = cmds.ikHandle(startJoint=fixedLengthProxies[0], endEffector=fixedLengthProxies[-1], curve=fittedCurveShape, createCurve=False, solver='ikSplineSolver')[0]
        ...
        control.attrState(fixedLengthProxies, ('t', 'r', 's'), *control.LOCK_HIDE)
        control.attrState(fixedLengthProxies, 'v', *control.HIDE)

    control.attrState(proxies, ('t', 'r', 's'), *control.LOCK_HIDE)
    control.attrState(proxies, 'v', *control.HIDE)
...
def buildNumControls(objs, numControls=3, **kw):
    ...
    ikHandle, ikEffector = cmds.ikHandle(sj=proxies[0], ee=proxies[-1], sol='ikSplineSolver', curve=fittedCurve, createCurve=False)
    ...
    for n in range(numControls):
        ...
        nControl = control.buildControl("control_%d" % n, halfWayPos, control.PivotModeDesc.MID, 'sphere', control.ColourDesc('darkblue'), constrain=False, parent=controlParent, scale=scale, asJoint=True)
